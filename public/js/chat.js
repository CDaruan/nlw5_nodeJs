...
document.querySelector("#start_chat").addEventListener("click", (event) => {
    ...
    socket.on("connect", () => {
        const params = { email:emailUser, text }
    ...
    socket.on("admin_send_to_client", (message) => {
        ...
        const temlate_admin = document.getElementById("admin-template").innerHTML;
        const rendered = Mustache.render(temlate_admin, {
            message_admin: message.text
        })
        document.getElementById("messages").innerHTML += rendered;
    })
...
document.getElementById("send_message_button").addEventListener("click", (event) => {
    ...
    const params = {
        text: text.value, socket_admin_id
    };
